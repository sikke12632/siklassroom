...
export const FINANCE_SCHEMA_STATEMENTS = [
  ...
  `CREATE TABLE IF NOT EXISTS finance_deposit_product_lifecycle_events (
    id TEXT PRIMARY KEY, class_id TEXT NOT NULL, product_id TEXT NOT NULL,
    revision INTEGER NOT NULL, action TEXT NOT NULL,
    capture_status TEXT NOT NULL, source_event_id TEXT,
    product_snapshot_json TEXT NOT NULL, actor_teacher_id TEXT NOT NULL,
    created_at INTEGER NOT NULL,
    FOREIGN KEY (source_event_id) REFERENCES finance_deposit_product_events(id),
    FOREIGN KEY (actor_teacher_id) REFERENCES teachers(id),
    FOREIGN KEY (product_id, class_id)
      REFERENCES finance_deposit_products(id, class_id),
    CONSTRAINT finance_deposit_product_lifecycle_id_ck CHECK (
      id = 'finance:deposit-product-lifecycle:' || product_id || ':' || revision
    ),
    CONSTRAINT finance_deposit_product_lifecycle_state_ck CHECK (
      action IN ('issued', 'opened', 'paused')
      AND ((action = 'issued' AND revision = 0)
        OR (action IN ('opened', 'paused') AND revision > 0))
      AND capture_status IN ('exact', 'legacy_event', 'legacy_current_only')
      AND ((capture_status = 'legacy_event' AND source_event_id IS NOT NULL)
        OR (capture_status IN ('exact', 'legacy_current_only')
          AND source_event_id IS NULL))
      AND json_valid(product_snapshot_json) = 1 AND created_at >= 0
    )
  )`,
  `CREATE UNIQUE INDEX IF NOT EXISTS
      finance_deposit_product_lifecycle_product_revision_uq
    ON finance_deposit_product_lifecycle_events(product_id, revision)`,
  `CREATE INDEX IF NOT EXISTS
      finance_deposit_product_lifecycle_class_created_idx
    ON finance_deposit_product_lifecycle_events(class_id, created_at, id)`,
  ...
  `CREATE TRIGGER IF NOT EXISTS finance_deposit_product_events_delete_guard
    ...
    BEGIN SELECT RAISE(ABORT, 'FINANCE_DEPOSIT_PRODUCT_EVENT_IMMUTABLE'); END`,
  `CREATE TRIGGER IF NOT EXISTS finance_deposit_product_lifecycle_insert_guard
          ...
          AND NEW.source_event_id IS NULL AND EXISTS (
            ...
            WHERE product.id = NEW.product_id
              ...
              AND CAST(json_extract(
                NEW.product_snapshot_json, '$.isOpen'
              ) AS INTEGER) = product.is_open
              AND json_extract(
                NEW.product_snapshot_json, '$.maturityInterestBps'
    ...
    END`,
  `CREATE TRIGGER IF NOT EXISTS finance_deposit_product_lifecycle_update_guard
    BEFORE UPDATE ON finance_deposit_product_lifecycle_events
    BEGIN
      SELECT RAISE(ABORT, 'FINANCE_DEPOSIT_PRODUCT_LIFECYCLE_IMMUTABLE');
    END`,
  `CREATE TRIGGER IF NOT EXISTS finance_deposit_product_lifecycle_delete_guard
  ...
  SELECT 'finance:deposit-product-lifecycle:' || product.id
         ...
         json_object(
           ...
           'isOpen', json(CASE product.is_open
  ...
  `CREATE TRIGGER IF NOT EXISTS finance_deposit_product_capture_issued_lifecycle
    ...
    END`,
  `CREATE TRIGGER IF NOT EXISTS finance_deposit_product_capture_state_lifecycle
  ...
  `CREATE TABLE IF NOT EXISTS finance_stock_tick_attempts (
    id TEXT PRIMARY KEY, class_id TEXT NOT NULL, retry_id TEXT NOT NULL,
    stock_id TEXT NOT NULL, stock_revision INTEGER NOT NULL,
    market_revision INTEGER NOT NULL, scheduled_tick_at INTEGER NOT NULL,
    stock_price_snapshot INTEGER NOT NULL,
    attempt_count INTEGER NOT NULL, error_code TEXT NOT NULL,
    failed_at INTEGER NOT NULL, next_attempt_at INTEGER NOT NULL,
    capture_status TEXT NOT NULL,
    FOREIGN KEY (stock_id, class_id) REFERENCES finance_stocks(id, class_id),
    CONSTRAINT finance_stock_tick_attempts_id_ck CHECK (
      id = 'finance:stock-tick-attempt:' || retry_id || ':' || attempt_count
    ),
    CONSTRAINT finance_stock_tick_attempts_state_ck CHECK (
      stock_revision >= 0 AND market_revision >= 0 AND scheduled_tick_at >= 0
      AND stock_price_snapshot BETWEEN 1 AND 1000000000
      AND attempt_count BETWEEN 1 AND 1000000
      AND LENGTH(TRIM(error_code)) BETWEEN 1 AND 100
      AND failed_at >= scheduled_tick_at AND next_attempt_at >= failed_at
      AND capture_status IN ('exact', 'legacy_latest')
    )
  )`,
  `CREATE UNIQUE INDEX IF NOT EXISTS finance_stock_tick_attempts_retry_attempt_uq
    ON finance_stock_tick_attempts(retry_id, attempt_count)`,
  `CREATE INDEX IF NOT EXISTS finance_stock_tick_attempts_class_failed_idx
    ON finance_stock_tick_attempts(class_id, failed_at, id)`,
  ...
  `CREATE TABLE IF NOT EXISTS finance_stock_news_applications (
    id TEXT PRIMARY KEY, class_id TEXT NOT NULL, stock_id TEXT NOT NULL,
    stock_event_id TEXT NOT NULL, stock_event_revision INTEGER NOT NULL,
    news_id TEXT NOT NULL, news_revision INTEGER NOT NULL,
    link_status TEXT NOT NULL, impact_bps INTEGER NOT NULL,
    news_payload_hash TEXT NOT NULL, applied_at INTEGER NOT NULL,
    recorded_at INTEGER NOT NULL,
    FOREIGN KEY (class_id) REFERENCES classes(id),
    FOREIGN KEY (stock_event_id) REFERENCES finance_stock_events(id),
    FOREIGN KEY (news_id) REFERENCES finance_stock_news(id),
    FOREIGN KEY (stock_id, class_id) REFERENCES finance_stocks(id, class_id),
    CONSTRAINT finance_stock_news_applications_id_ck CHECK (
      id = 'finance:stock-news-application:' || stock_id || ':' || news_id
    ),
    CONSTRAINT finance_stock_news_applications_state_ck CHECK (
      link_status IN ('exact', 'legacy_inferred')
      AND stock_event_revision > 0 AND news_revision = 0
      AND impact_bps BETWEEN -10000 AND 10000
      AND LENGTH(TRIM(news_payload_hash)) BETWEEN 8 AND 500
      AND applied_at >= 0 AND recorded_at >= applied_at
    )
  )`,
  `CREATE UNIQUE INDEX IF NOT EXISTS
      finance_stock_news_applications_stock_news_uq
    ON finance_stock_news_applications(stock_id, news_id)`,
  `CREATE UNIQUE INDEX IF NOT EXISTS
      finance_stock_news_applications_event_news_uq
    ON finance_stock_news_applications(stock_event_id, news_id)`,
  `CREATE INDEX IF NOT EXISTS finance_stock_news_applications_class_applied_idx
    ON finance_stock_news_applications(class_id, applied_at, id)`,
  ...
  `CREATE TABLE IF NOT EXISTS finance_stock_liquidation_events (
    id TEXT PRIMARY KEY, class_id TEXT NOT NULL, operation_id TEXT NOT NULL,
    revision INTEGER NOT NULL, action TEXT NOT NULL, stock_id TEXT NOT NULL,
    student_id TEXT NOT NULL, actor_teacher_id TEXT NOT NULL,
    chunk_id TEXT, chunk_index INTEGER, trade_id TEXT,
    request_idempotency_key TEXT NOT NULL,
    request_payload_hash TEXT NOT NULL, reason TEXT NOT NULL,
    initial_quantity INTEGER NOT NULL,
    remaining_quantity INTEGER NOT NULL, sold_quantity INTEGER NOT NULL,
    completed_chunk_count INTEGER NOT NULL,
    quantity_delta INTEGER NOT NULL, wallet_delta INTEGER NOT NULL,
    total_gross_amount INTEGER NOT NULL, total_fee_amount INTEGER NOT NULL,
    total_wallet_delta INTEGER NOT NULL,
    total_cost_basis_removed INTEGER NOT NULL,
    total_realized_gain INTEGER NOT NULL, created_at INTEGER NOT NULL,
    FOREIGN KEY (student_id) REFERENCES students(id),
    FOREIGN KEY (actor_teacher_id) REFERENCES teachers(id),
    FOREIGN KEY (chunk_id) REFERENCES finance_stock_liquidation_chunks(id),
    FOREIGN KEY (trade_id) REFERENCES finance_stock_trades(id),
    FOREIGN KEY (operation_id, class_id)
      REFERENCES finance_stock_liquidation_operations(id, class_id),
    FOREIGN KEY (stock_id, class_id) REFERENCES finance_stocks(id, class_id),
    CONSTRAINT finance_stock_liquidation_events_id_ck CHECK (
      id = 'finance:stock-liquidation-event:' || operation_id || ':' || revision
        || ':' || action
    ),
    CONSTRAINT finance_stock_liquidation_events_text_ck CHECK (
      LENGTH(TRIM(request_idempotency_key)) BETWEEN 8 AND 200
      AND LENGTH(TRIM(request_payload_hash)) BETWEEN 8 AND 500
      AND LENGTH(TRIM(reason)) BETWEEN 2 AND 300
    ),
    CONSTRAINT finance_stock_liquidation_events_action_ck CHECK (
      action IN ('started', 'chunk_completed', 'completed', 'cancelled')
    ),
    CONSTRAINT finance_stock_liquidation_events_amount_ck CHECK (
      revision BETWEEN 0 AND 3
      AND initial_quantity BETWEEN 1 AND 1000000000
      AND remaining_quantity BETWEEN 0 AND 1000000000
      AND sold_quantity BETWEEN 0 AND 1000000000
      AND initial_quantity = remaining_quantity + sold_quantity
      AND completed_chunk_count BETWEEN 0 AND 2
      AND quantity_delta BETWEEN 0 AND 1000000000
      AND wallet_delta BETWEEN 0 AND 1000000000
      AND total_gross_amount BETWEEN 0 AND 1111111111
      AND total_fee_amount BETWEEN 0 AND total_gross_amount
      AND total_wallet_delta = total_gross_amount - total_fee_amount
      AND total_wallet_delta BETWEEN 0 AND 1000000000
      AND total_cost_basis_removed BETWEEN 0 AND 1000000000
      AND total_realized_gain = total_wallet_delta - total_cost_basis_removed
      AND created_at >= 0
    ),
    CONSTRAINT finance_stock_liquidation_events_state_ck CHECK (
      (action = 'started' AND revision = 0
        AND chunk_id IS NULL AND chunk_index IS NULL AND trade_id IS NULL
        AND remaining_quantity > 0 AND sold_quantity = 0
        AND completed_chunk_count = 0
        AND quantity_delta = 0 AND wallet_delta = 0
        AND total_gross_amount = 0
        AND total_fee_amount = 0 AND total_wallet_delta = 0
        AND total_cost_basis_removed = 0 AND total_realized_gain = 0)
      OR (action = 'chunk_completed'
        AND chunk_id IS NOT NULL AND chunk_index IS NOT NULL
        AND trade_id IS NOT NULL
        AND revision = completed_chunk_count
        AND completed_chunk_count BETWEEN 1 AND 2 AND sold_quantity > 0
        AND chunk_index = completed_chunk_count - 1
        AND quantity_delta > 0 AND wallet_delta > 0)
      OR (action = 'completed'
        AND chunk_id IS NOT NULL AND chunk_index IS NOT NULL
        AND trade_id IS NOT NULL
        AND revision = completed_chunk_count
        AND completed_chunk_count BETWEEN 1 AND 2
        AND chunk_index = completed_chunk_count - 1
        AND remaining_quantity = 0 AND sold_quantity > 0
        AND quantity_delta = 0 AND wallet_delta = 0)
      OR (action = 'cancelled'
        AND chunk_id IS NULL AND chunk_index IS NULL AND trade_id IS NULL
        AND revision = completed_chunk_count + 1 AND remaining_quantity > 0
        AND quantity_delta = 0 AND wallet_delta = 0)
    )
  )`,
  `CREATE UNIQUE INDEX IF NOT EXISTS
      finance_stock_liquidation_events_operation_revision_action_uq
    ON finance_stock_liquidation_events(operation_id, revision, action)`,
  `CREATE UNIQUE INDEX IF NOT EXISTS
      finance_stock_liquidation_events_class_action_request_uq
    ON finance_stock_liquidation_events(
      class_id, action, request_idempotency_key
    )`,
  `CREATE INDEX IF NOT EXISTS
      finance_stock_liquidation_events_class_created_idx
    ON finance_stock_liquidation_events(class_id, created_at, id)`,
  ...
  `CREATE TRIGGER IF NOT EXISTS finance_stock_markets_update_guard
    ...
    END`,
  `DROP TRIGGER IF EXISTS finance_stock_market_system_class_guard`,
  `CREATE TRIGGER IF NOT EXISTS finance_stock_market_system_class_guard
  ...
  `CREATE TRIGGER IF NOT EXISTS finance_stocks_management_update_guard
    ...
    END`,
  `DROP TRIGGER IF EXISTS finance_stock_system_class_guard`,
  `CREATE TRIGGER IF NOT EXISTS finance_stock_system_class_guard
    BEFORE UPDATE ON finance_stocks
    WHEN NEW.last_trade_id IS OLD.last_trade_id
  ...
  `CREATE TRIGGER IF NOT EXISTS finance_stock_tick_retries_update_cleanup
    ...
    BEGIN
      DELETE FROM finance_stock_tick_retries WHERE id = NEW.id;
    END`,
  `CREATE TRIGGER IF NOT EXISTS finance_stock_tick_attempts_insert_guard
    ...
    BEGIN
      ...
      SELECT CASE WHEN NOT EXISTS (
        SELECT 1 FROM finance_stock_tick_retries retry
        JOIN finance_stocks stock
          ON stock.id = retry.stock_id AND stock.class_id = retry.class_id
        WHERE retry.id = NEW.retry_id AND retry.class_id = NEW.class_id
    ...
    END`,
  `CREATE TRIGGER IF NOT EXISTS finance_stock_tick_attempts_update_guard
    BEFORE UPDATE ON finance_stock_tick_attempts
    BEGIN SELECT RAISE(ABORT, 'FINANCE_STOCK_TICK_ATTEMPT_IMMUTABLE'); END`,
  `CREATE TRIGGER IF NOT EXISTS finance_stock_tick_attempts_delete_guard
    BEFORE DELETE ON finance_stock_tick_attempts
    BEGIN SELECT RAISE(ABORT, 'FINANCE_STOCK_TICK_ATTEMPT_IMMUTABLE'); END`,
  `INSERT OR IGNORE INTO finance_stock_tick_attempts (
    id, class_id, retry_id, stock_id, stock_revision, market_revision,
    scheduled_tick_at, stock_price_snapshot, attempt_count, error_code, failed_at,
    next_attempt_at, capture_status
  )
  SELECT 'finance:stock-tick-attempt:' || retry.id || ':' || retry.attempt_count,
         ...
         retry.attempt_count, retry.last_error_code, retry.last_failed_at, retry.next_attempt_at,
         'legacy_latest'
  FROM finance_stock_tick_retries retry
  JOIN finance_stocks stock
    ON stock.id = retry.stock_id AND stock.class_id = retry.class_id`,
  `CREATE TRIGGER IF NOT EXISTS finance_stock_tick_capture_first_attempt
    AFTER INSERT ON finance_stock_tick_retries
    WHEN EXISTS (
      SELECT 1
      FROM finance_stock_markets market
    ...
    BEGIN
      ...
      SELECT
        'finance:stock-tick-attempt:' || NEW.id || ':' || NEW.attempt_count,
    ...
    END`,
  `CREATE TRIGGER IF NOT EXISTS finance_stock_tick_capture_later_attempt
  ...
  `CREATE TRIGGER IF NOT EXISTS finance_stock_events_insert_guard
    ...
    END`,
  `DROP TRIGGER IF EXISTS finance_stock_system_event_class_guard`,
  `CREATE TRIGGER IF NOT EXISTS finance_stock_system_event_class_guard
    BEFORE INSERT ON finance_stock_events
  ...
  `CREATE TRIGGER IF NOT EXISTS finance_stock_news_capture_transition_event
    ...
    END`,
  `CREATE TRIGGER IF NOT EXISTS finance_stock_news_applications_insert_guard
    ...
    END`,
  `CREATE TRIGGER IF NOT EXISTS finance_stock_news_applications_update_guard
    BEFORE UPDATE ON finance_stock_news_applications
    BEGIN
      SELECT RAISE(ABORT, 'FINANCE_STOCK_NEWS_APPLICATION_IMMUTABLE');
    END`,
  `CREATE TRIGGER IF NOT EXISTS finance_stock_news_applications_delete_guard
  ...
  WHERE published.action = 'published' AND published.revision = 0
    AND NOT EXISTS (
      SELECT 1 FROM system_migrations migration
      WHERE migration.key = 'finance-stock-news-applications-v1'
    )`,
  ...
  `CREATE TRIGGER IF NOT EXISTS finance_stock_liquidation_chunks_delete_guard
    ...
    BEGIN SELECT RAISE(ABORT, 'FINANCE_STOCK_LIQUIDATION_CHUNK_IMMUTABLE'); END`,
  `DROP TRIGGER IF EXISTS finance_stock_liquidation_capture_started_event`,
  `DROP TRIGGER IF EXISTS finance_stock_liquidation_capture_progress_events`,
  `DROP TRIGGER IF EXISTS finance_stock_liquidation_capture_cancelled_event`,
  `DROP TRIGGER IF EXISTS finance_stock_liquidation_events_insert_guard`,
  `CREATE TRIGGER finance_stock_liquidation_events_insert_guard
        ...
        WHERE operation.id = NEW.operation_id
          ...
          AND (
            ...
            OR (NEW.action = 'cancelled'
              ...
              AND NEW.request_payload_hash
                = operation.cancellation_payload_hash
    ...
    END`,
  `CREATE TRIGGER IF NOT EXISTS finance_stock_liquidation_events_update_guard
    BEFORE UPDATE ON finance_stock_liquidation_events
    BEGIN
      SELECT RAISE(ABORT, 'FINANCE_STOCK_LIQUIDATION_EVENT_IMMUTABLE');
    END`,
  `CREATE TRIGGER IF NOT EXISTS finance_stock_liquidation_events_delete_guard
  ...
  SELECT 'finance:stock-liquidation-event:' || operation.id || ':0:started',
         operation.class_id, operation.id, 0, 'started', operation.stock_id,
         operation.student_id, operation.teacher_id, NULL, NULL, NULL,
         operation.root_idempotency_key, operation.payload_hash,
         operation.intervention_reason, operation.initial_quantity,
         operation.initial_quantity, 0, 0, 0, 0, 0, 0, 0, 0, 0,
  ...
  JOIN finance_stock_liquidation_chunks chunk
    ON chunk.operation_id = operation.id AND chunk.class_id = operation.class_id
  JOIN finance_stock_trades trade
    ON trade.id = chunk.trade_id AND trade.class_id = chunk.class_id`,
  `INSERT OR IGNORE INTO finance_stock_liquidation_events (
    id, class_id, operation_id, revision, action, stock_id,
  ...
  `CREATE TRIGGER IF NOT EXISTS finance_payroll_classes_archive_guard
    ...
    BEGIN SELECT RAISE(ABORT, 'FINANCE_PAYROLL_PENDING_CLASS'); END`,
  `CREATE TRIGGER IF NOT EXISTS finance_payroll_students_exclude_guard
    ...
    BEGIN SELECT RAISE(ABORT, 'FINANCE_PAYROLL_PENDING_STUDENT'); END`,
  `CREATE TRIGGER IF NOT EXISTS finance_salary_settings_pending_payroll_guard
    ...
    BEGIN SELECT RAISE(ABORT, 'FINANCE_PAYROLL_PENDING_SETTINGS'); END`,
  `CREATE TRIGGER IF NOT EXISTS finance_denominations_pending_payroll_guard
    ...
    BEGIN SELECT RAISE(ABORT, 'FINANCE_PAYROLL_PENDING_DENOMINATIONS'); END`,
  `CREATE TRIGGER IF NOT EXISTS finance_payroll_runs_prepare_guard
    ...
    BEGIN SELECT RAISE(ABORT, 'FINANCE_PAYROLL_PREPARE_STALE'); END`,
  `CREATE TRIGGER IF NOT EXISTS finance_payroll_items_prepare_guard
    ...
    BEGIN SELECT RAISE(ABORT, 'FINANCE_PAYROLL_RECIPIENT_STALE'); END`,
