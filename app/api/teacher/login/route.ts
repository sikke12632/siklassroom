...
export async function POST(request: Request) {
  try {
    ...
    const teacher = await database().prepare(
      `SELECT id, email, password_hash, status FROM teachers WHERE email = ?`,
    ).bind(email).first<{ id: string; email: string; password_hash: string; status: string }>();
    ...
    return json({ teacher: { id: teacher.id, email: teacher.email } }, 200, { "Set-Cookie": session.cookie });
  } catch (error) {
