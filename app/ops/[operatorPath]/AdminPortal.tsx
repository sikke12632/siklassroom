...
function SchoolManager({ request, onError, onMessage }: { request: Requester; onError: (v: string) => void; onMessage: (v: string) => void }) {
  ...
  return <section className="admin-section school-review-list">{items.map((item) => <article key={item.id} className="admin-review-card"><div><span className={`status-badge status-${item.status}`}>{item.status}</span><h2>{item.entered_name}</h2><p>{item.province_name} · {item.school_level} · {item.district_or_address || "지역 정보 없음"}</p><small>{item.teacher_email} · {when(item.created_at)}</small>{item.note && <blockquote>{item.note}</blockquote>}</div>{item.status === "pending" && <div className="admin-review-actions"><select aria-label={`${item.entered_name} 요청에 연결할 공식 학교`} value={selected[item.id] || ""} onChange={(e) => setSelected((current) => ({ ...current, [item.id]: e.target.value }))}><option value="">기존 공식 학교 선택</option>{schools.filter((school) => school.province_name === item.province_name && school.school_level === item.school_level).map((school) => <option value={school.id} key={school.id}>{school.official_name} · {school.district_name || ""}</option>)}</select><button disabled={!selected[item.id]} onClick={() => review(item.id, "link")}>공식 학교와 연결</button><button onClick={() => review(item.id, "approve_new")}>신규 학교 승인</button><button className="danger-link" onClick={() => review(item.id, "reject")}>반려</button></div>}</article>)}</section>;
}
