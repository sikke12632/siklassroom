  ...
  return (
    <main className="activation-page">
      ...
      <section className="activation-card">
        {loading ? <div className="student-loading inline"><span>QR</span><p>내 카드를 확인하고 있어요</p></div> : error && !info ? <div className="expired-qr"><span>!</span><h1>이 QR은 사용할 수 없어요</h1><Notice message={error} tone="error" /><p>QR이 만료되었거나 선생님이 새 QR을 발급했을 수 있어요. 현재 카드를 확인해 주세요.</p><a className="button button-light" href="/student">학생 로그인으로</a></div> : info && <>
          ...
          <div className="identity-check"><span className="check-mark">✓</span><small>{info.isReturning ? "비밀번호를 다시 만들 학생" : "이 QR의 주인"}</small><h1>{info.student.student_number}번 {info.student.official_name}</h1><p>{info.student.school_name} · {info.student.grade}학년 {info.student.class_number}반</p></div>
          <form onSubmit={complete} className="activation-form">
            ...
            <button className="button button-student button-large" disabled={busy}>{busy ? "안전하게 저장 중…" : info.isReturning ? "새 비밀번호로 바꾸기 →" : "등록하고 바로 들어가기 →"}</button>
          </form>
        </>}
      </section>
    </main>
