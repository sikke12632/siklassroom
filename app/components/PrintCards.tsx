  ...
  return (
    <div ref={overlayRef} className="print-overlay" role="dialog" aria-modal="true" aria-label="학생 QR 카드 인쇄 미리보기">
      <div className="print-toolbar no-print">
        <div><strong>QR 카드 인쇄 미리보기</strong><span role={generationError ? "alert" : undefined}>{generationError || `${cards.length}장 · 교사가 사용 중지하기 전까지 같은 카드로 계속 로그인할 수 있어요.`}</span></div>
      ...
      <main className="print-sheet">
        {cards.map((card) => (
          <article className="qr-card" key={card.id}>
            ...
            <div className="one-time-key">교사가 사용 중지하기 전까지 다시 쓰는 개인 카드</div>
          </article>
        ))}
      </main>
    </div>
